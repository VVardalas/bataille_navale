...
public class Partie {
	...
	private static List<Joueur> listeJoueurs = new ArrayList<Joueur>(); //vous gérez à plus de deux joueurs ici (et dans "executerUnTir"), mais pas dans Joueur, c'est dommage
		...
		do
		{
			...
			for (Joueur joueurCourant : listeJoueurs)
			{
				...
				for(Integer idBateauCourant : joueurCourant.getListeBateaux().keySet())
				{
					...
					do{
						System.out.println(" Entrer la position X de d�but : ");
						try
						{
							xDebut = s.nextInt();
							ok = true;
					...
					do{
						System.out.println(" Entrer la position Y de d�but : ");
						try
						{
							yDebut = s.nextInt();
							ok = true;
